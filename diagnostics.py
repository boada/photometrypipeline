...
def insert_into_summary(text):
    ...
    """
    append_website(_pp_conf.diagnostics_summary,
                   text + '\n',
...
def create_index(filenames, directory, obsparam, display=False):
    ...
    for idx, filename in enumerate(filenames):
        ...
        median = numpy.median(imgdat[int(imgdat.shape[1] * 0.25):int(
            imgdat.shape[1] * 0.75), int(imgdat.shape[0] * 0.25):int(
                imgdat.shape[0] * 0.75)])
        std = numpy.std(imgdat[int(imgdat.shape[1] * 0.25):int(imgdat.shape[
            1] * 0.75), int(imgdat.shape[0] * 0.25):int(imgdat.shape[0] *
                                                        0.75)])

        downscale = 2.  # scale down image by this factor
        fig = plt.figure(figsize=(old_div(header[obsparam['extent'][0]],
                                          (downscale * 100)),
                                  old_div(header[obsparam['extent'][1]],
                                          (downscale * 100))),
                         dpi=downscale * 100)
...
def add_registration(data, extraction_data):
    ...
    for dat in extraction_data:
        ...
        if refcat.shape[0] > 0:
            w = wcs.WCS(header)
            world_coo = numpy.array(list(zip(refcat['ra.deg'], refcat[
                'dec.deg'])))
            img_coo = w.wcs_world2pix(world_coo, True)
            img_coo = [
                c for c in img_coo
                if (c[0] > 0 and c[1] > 0 and c[0] < header[obsparam['extent'][
                    0]] and c[1] < header[obsparam['extent'][1]])
            ]
    ...
    append_website(_pp_conf.index_filename,
                   html,
...
def add_photometry(data, extraction):
    ...
    if not parameters['target_only']:
        plt.errorbar(parameters['aprad'],
                     data['background_flux'][0], data['background_flux'][1],
                     color='black', linewidth=1, label='background objects')
    if not parameters['background_only']:
        plt.errorbar(parameters['aprad'],
                     data['target_flux'][0], data['target_flux'][1],
                     color='red', linewidth=1, label='target')
    ...
    plt.scatter(frame_midtimes, fwhm, marker='o', color='black')
    ...
    append_website(
        _pp_conf.index_filename,
        html,
        replace_below=(
            "<H2>Photometric Calibration" + " - Aperture Size </H2>\n"))
...
def add_calibration(data):
    ...
    for idx, cat in enumerate(data['catalogs']):
        ...
        ax1.set_title('%s: %s-band from %s' % (
            cat.catalogname, data['filtername'], data['ref_cat'].catalogname))
        ...
        ax1.errorbar(x, [clipping_steps[i][0]
                         for i in range(len(clipping_steps))],
                     yerr=[clipping_steps[i][1]
                           for i in range(len(clipping_steps))],
                     color='red')
        ...
        ax2 = ax1.twinx()
        ax2.plot(x, [clipping_steps[i][2] for i in range(len(clipping_steps))],
                 color='blue')
    ...
    for dat in data['zeropoints']:
        ...
        if len(dat['match'][0][3]) > 0 and len(dat['match'][0][4]) > 0:
            ...
            for i in range(len(dat['zp_usedstars'])):
                plt.annotate(
                    str(i + 1),
                    xy=(img_coo[i][0] + 30, img_coo[i][1]),
                    color='red',
                    horizontalalignment='left',
                    verticalalignment='center')

        plt.savefig(catframe, format='png', bbox_inches='tight', pad_inches=0)
        plt.close()
    ...
    append_website(
        ...
        html,
        replace_below=(
            "<H2>Photometric Calibration " + "- Catalog Match </H2>\n"))
...
def add_results(data):
    ...
    for target in data['targetnames']:
        ...
        data['lightcurveplots'][target] = (
            '.diagnostics/' + '%s.png' % target.replace(' ', '_'))
    ...
    for target in data['targetnames']:
        ...
        for dat in data[target]:
            ...
            image_coords = w.wcs_world2pix(
                numpy.array([[dat[1], dat[2]]]), True)
            ...
            thumbdata = composite[int(boxsize + obj_y - old_div(
                boxsize, 2)):int(boxsize + obj_y + old_div(boxsize, 2)), int(
                    boxsize + obj_x - old_div(boxsize, 2)):int(
                        boxsize + obj_x + old_div(boxsize, 2))]

            ## run statistics over center of the frame around the target
            if thumbdata.shape[0] > 0 and thumbdata.shape[1] > 0:
                median = numpy.median(thumbdata[old_div(
                    boxsize, 2) - 20:old_div(boxsize, 2) + 20, old_div(
                        boxsize, 2) - 20:old_div(boxsize, 2) + 20])
                std = numpy.std(thumbdata[old_div(boxsize, 2) - 20:old_div(
                    boxsize, 2) + 20, old_div(boxsize, 2) - 20:old_div(
                        boxsize, 2) + 20])
                maxval = numpy.max(thumbdata[old_div(boxsize, 2) - 20:old_div(
                    boxsize, 2) + 20, old_div(boxsize, 2) - 20:old_div(
                        boxsize, 2) + 20])
            else:
            ...
            plt.annotate('%s\n%5.3f+-%5.3f mag' % (fitsfilename, dat[7],
                                                   dat[8]), (3, 10),
                         color='white')
    ...
    for target in data['targetnames']:
        ...
        for idx, plts in enumerate(data['thumbnailplots'][target]):
            html += "<P>%s<IMG ID=\"%s\" SRC=\"%s\">\n" % (
                plts[0], data[target][idx][10],
                plts[1].split('.diagnostics/')[1])
        filename = '.diagnostics/' + target.replace(' ',
                                                    '_') + '_' + 'results.html'
