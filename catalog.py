...
class catalog(object):
    ...
    def add_field(self, field_name, field_array, field_type='D'):
        ...
        return self.data.add_column(Column(field_array,
                                           name=field_name,
                                           format=field_type))
    ...
    def add_fields(self, field_names, field_arrays, field_types):
        ...
        for i in range(len(field_names)):
            self.data.add_column(Column(
                numpy.array(field_arrays[i]),
                name=field_names[i],
    ...
    def download_catalog(self,
        ...
        if self.catalogname == 'GAIA':
            ...
            self.data.add_column(Column(
                numpy.ones(len(self.data)) * 2457023.5,
                name='epoch_jd',
                unit=u.day))
            ...
            ra_err = abs(self.data['errMaj'] * numpy.cos(arc_xopt) * numpy.cos(
                self.data['errPA'].to(u.rad)) - self.data['errMin'] *
                         numpy.sin(arc_xopt) * numpy.sin(self.data['errPA'].to(
                             u.rad)))
            self.data.add_column(
                Column(data=ra_err * 1000,
                       name='e_ra.deg',
                       unit=u.mas),
                index=2)
            ...
            dec_err = abs(self.data['errMaj'] * numpy.cos(arc_yopt) *
                          numpy.sin(self.data['errPA'].to(u.rad)) + self.data[
                              'errMin'] * numpy.sin(arc_yopt) * numpy.cos(
                                  self.data['errPA'].to(u.rad)))
            self.data.add_column(
                Column(data=dec_err * 1000,
                       name='e_dec.deg',
                       unit=u.mas),
                index=3)
            ...
            self.data.add_column(
                Column(data=self.data['sigm'].data,
                       name='e_ra.deg',
            ...
            # photometric catalog
            vquery = Vizier(
                columns=['recno', 'RAJ2000', 'DEJ2000', 'e_RAJ2000',
                         'e_DEJ2000', 'Vmag', 'e_Vmag', 'Bmag', 'e_Bmag',
                         "g'mag", "e_g'mag", "r'mag", "e_r'mag", "i'mag",
                         "e_i'mag"],
                column_filters={"Vmag":
                                ("<%f" % max_mag)},
                row_limit=max_sources)
            ...
            vquery = Vizier(
                columns=['SDSS9', 'RAJ2000', 'DEJ2000', 'e_RAJ2000',
                         'e_DEJ2000', 'umag', 'e_umag', 'gmag', 'e_gmag',
    ...
    def write_ldac(self, ldac_filename):
        ...
        ### create header table
        hdr_col = fits.Column(name='Field Header Card',
                              format='1680A',
        ...
        ### create data table
        colname_dic = {'ra.deg': 'XWIN_WORLD',
                       'dec.deg': 'YWIN_WORLD',
                       'e_ra.deg': 'ERRAWIN_WORLD',
                       'e_dec.deg': 'ERRBWIN_WORLD',
                       'mag': 'MAG'}
        format_dic = {'ra.deg': '1D',
                      'dec.deg': '1D',
                      'e_ra.deg': '1E',
                      'e_dec.deg': '1E',
                      'mag': '1E'}
        ...
        data_cols.append(fits.Column(name='OBSDATE',
                                     ...
                                     format='1D',
                                     unit='yr',
                                     array=numpy.ones(len(self.data)) *
                                     2015.0))
    ...
    def write_database(self, filename):
        ...
        db.execute("INSERT INTO header VALUES (?,?,?,?,?,?,?)",
                   (self.catalogname, self.origin, self.history, self.magsys,
                    self.obstime[0], self.obstime[1], self.obj))
        ...
        sqltypes = {numpy.float32: numpy.float64,
    ...
    def read_database(self, filename):
        ...
        columns = [fits.Column(name=fieldnames[idx],
                               ...
                               array=data[idx]) for idx in range(len(data))]
    ...
    def transform_filters(self, targetfilter):
        ...
        if (('SDSS' in self.catalogname) and
            ...
            mags = numpy.array([self['gmag'], self['rmag'], self['imag'],
                                self['e_gmag'], self['e_rmag'], self['e_imag'],
                                self['umag'], self['e_umag']])
            ...
            resid = numpy.sqrt((old_div((ri + param[0] * gr - param[0] * param[
                1]), (param[0]**2 + 1)))**2 + (param[0] * (ri + param[
                    0] * gr - param[0] * param[1]) / (param[0]**2 + 1) + param[
                        1] - gr)**2)
        ...
        # todo: include g magnitudes and account for color
        elif ('URAT' in self.catalogname or 'APASS' in self.catalogname and
              (targetfilter == 'R' or targetfilter == 'I') and
              self.magsystem == 'Vega'):
            ...
            ### transformations based on Chonis & Gaskell 2008, AJ, 135
            mags = numpy.array([self['rmag'], self['imag'], self['e_rmag'],
                                self['e_imag']])
        ...
        ### 2MASS to UKIRT YZJHK)
           ...
           (self.magsystem == 'Vega'):
            ...
            mags = [self['Jmag'], self['Hmag'], self['Kmag'], self['e_Jmag'],
                    self['e_Hmag'], self['e_Kmag']]
            lbl = {'_Ymag': 0,
                   '_e_Ymag': 1,
                   '_Zmag': 2,
                   '_e_Zmag': 3,
            ...
            for idx in range(self.shape[0]):
                ...
                if keep:
                    nmags[lbl['_Bmag']][idx]   = mags[0][idx] + 1.7495*cidx**3 \
                                                - 2.7785*cidx**2 \
                                                + 5.215*cidx + 0.1980
                    nmags[lbl['_e_Bmag']][idx] = numpy.sqrt(0.08 * 0.08 + mags[
                        2][idx]**2)

                    nmags[lbl['_Vmag']][idx]   = mags[0][idx] + 1.4688*cidx**3 \
                                                - 2.3250*cidx**2 \
                                                + 3.5143*cidx + 0.1496
                    nmags[lbl['_e_Vmag']][idx] = numpy.sqrt(0.05 * 0.05 + mags[
                        2][idx]**2)

                    nmags[lbl['_Rmag']][idx]   = mags[0][idx] + 1.1230*cidx**3 \
                                                - 1.7849*cidx**2 \
                                                + 2.5105*cidx + 0.1045
                    nmags[lbl['_e_Rmag']][idx] = numpy.sqrt(0.08 * 0.08 + mags[
                        2][idx]**2)

                    nmags[lbl['_Imag']][idx]   = mags[0][idx] + 0.2963*cidx**3 \
                                                - 0.4866*cidx**2 \
                                                + 1.2816*cidx + 0.0724
                    nmags[lbl['_e_Imag']][idx] = numpy.sqrt(0.03 * 0.03 + mags[
                        2][idx]**2)
                else:
                    for mag in lbl.keys():
                        nmags[lbl[mag]][idx] = 99
    ...
    def match_with(self,
        ...
        this_tree = spatial.KDTree(list(zip(self[match_keys_this_catalog[
            0]].data, self[match_keys_this_catalog[1]].data)))
        ...
        if tolerance is not None:
            ...
            indices_this_catalog = [x for x in range(len(match))
                                    if len(match[x]) == 1]
            indices_other_catalog = [x[
                0] for x in list(filter((lambda x: len(x) == 1), match))]
            ...
            for target_idx in range(self.shape[0]):
                ...
                if len(other_cat_indices) > 0:
                    # find closest match
                    min_idx = other_cat_indices[numpy.argmin([match[0][
                        i] for i in other_cat_indices])]
